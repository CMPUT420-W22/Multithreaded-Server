...
#include "timer.h"
using namespace std;
// https://www.youtube.com/watch?v=0E9sq-w1TWU&t=8s&ab_channel=MasterMirror
// implemented ideas with video from youtube
...
void *ServerEcho(void *args)
{
    ...
    //**critical section**//
    pthread_mutex_lock(&mutex[rqst.pos]);
    ...
    request++;
    pthread_mutex_unlock(&mutex[rqst.pos]);
    //**Critical Section**//
...
int main(int argc, char *argv[])
{
    ...
    mutex = (pthread_mutex_t*)malloc(elements * sizeof(pthread_mutex_t));

    for (int i = 0; i < elements; i++)
    {
        theArray[i] = (char *)malloc(COM_BUFF_SIZE * sizeof(char));
        //initalizing all the mutex lock for each element
